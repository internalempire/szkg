...
let overlayGeometry = null, overlayAngle = null;
let paperMeta = {};
let explorer = null, refreshInProgress = false;
...
function resetSelection() {
  activeTopic = null; previewTopic = null; deselectNode(); clearHighlight(false); hidePanels();
...
function showInfo(id, boxId) { explorer.showInfo(id, boxId); }
...
function selectPaper(id) {
  ...
  applyHighlight(new Set([id, ...graph.neighbors(id)]));
  showInfo(id, "info"); scheduleTopicOverlay(); explorer.update();
}
...
function createExplorer(data) {
  explorer = window.PaperExplorer.create({
    ...
    restoreCamera(camera) {
      // Supersede any running animation; duration zero divides by zero in Sigma 3.
    ...
    resize() { renderer.resize(); scheduleTopicOverlay(); },
    search(ids) {
      activeTopic = null; previewTopic = null; deselectNode(); hidePanels();
    ...
    open(id) {
      ...
      if (point) renderer.getCamera().animate({ x: point.x, y: point.y,
    ...
    compare(id, edge) {
      ...
      selectedEdge = edge; refreshRenderer(); showInfo(id, "info2");
    },
    reveal(id) {
    ...
    setTopicVisible(id, visible) {
      ...
      if (!visible && (activeTopic === id ||
          (selectedNode && graph.getNodeAttribute(selectedNode, "cluster") === id))) resetSelection();
    ...
    preview(id) {
      previewTopic = id;
      id === null ? restoreHighlight()
        : applyHighlight(new Set(graph.filterNodes((_key, node) => node.cluster === id)));
      scheduleTopicOverlay();
    },
    clearFilters() {
      hiddenTopics.clear(); showWeakAssignments = true; resetSelection(); updateVisibility();
    },
    fit() { renderer.getCamera().animatedReset({ duration: 250 }); },
    closePanel(box) {
      if (box === "info") resetSelection();
      else { document.getElementById("info2").classList.add("hidden"); selectedEdge = null; refreshRenderer(); }
    },
  });
...
function updateTopicOverlay() {
  ...
  if (!overlayGeometry || angle !== overlayAngle) {
    const nodes = graph.mapNodes((id, attributes) => ({ id, x: attributes.x, y: attributes.y,
      cluster: attributes.cluster, weak: attributes.weak_assignment, hidden: attributes.hidden }));
...
async function refreshData() {
  ...
  try {
    ...
    document.getElementById("info2").classList.add("hidden"); selectedEdge = null;
    explorer.setData(data, currentTopics, paperMeta);
    if (selectedNode) showInfo(selectedNode, "info");
    updateStatistics(); updateVisibility(); createTopicLabels(currentTopics); restoreHighlight();
...
async function start() {
  try {
    ...
    window.MapStatus.render(data.status);
    createRenderer(); createExplorer(data); bindRendererEvents(); bindControls();
