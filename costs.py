...
PRICE_PER_MILLION_TOKENS: dict[str, float] = {
    ...
    # Snapshot estimate only; gateway/provider billing can differ.
...
def count_tokens(text: str, model: str) -> int:
    ...
    try:
        encoding = tiktoken.encoding_for_model(model.removeprefix("openai/"))
    except KeyError:
...
def estimate_cost(tokens: int, model: str) -> float | None:
    ...
    return tokens / 1_000_000 * price if price is not None else None
...
class CostTracker:
    ...
    def record(self, tokens: int, paper_count: int) -> None:
        ...
        self.total_cost = self.total_cost + cost if self.total_cost is not None and cost is not None else None
